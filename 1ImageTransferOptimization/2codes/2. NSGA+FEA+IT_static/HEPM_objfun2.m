%*************************************************************************
% This is the function to calculate the NSGA objective from input using FEM+IT
% 	01-Jun-2020 sichao yang
%*************************************************************************
function [y, cons, varino, time] = HEPM_objfun2(x,inputfile,datafile,gen, root)
cons = [];
Io_intval = 50;       % scan interval for opt current angle
ang = [10:Io_intval:50]/180*pi;     % scan angle for current advancing
isotest = 0;           % check the ouput without given inputs from parent files
Dri=0.030;             %[m] inner rotor diameter  for weight calculation
p=4;                   %[-] pole pair
NoSlot=48;
...
la=151/1000*ones(caseNoin,1);   %[m] stack length used in FEM

%% A. map pop. variable to FEM inputs
%{ 
    variables: 'magThick [mm]' 'Tooth width [mm]' 'Magnet angle [deg]' 'Rotor outer diameter [mm]'
    'magWidth[mm]' 'Position of mag [mm]' 'CorebackWidth [mm]' 'Ja [A_per_m2]'
%}
if isotest == 0
    varin(:,1) = x(:,1)*0.5 + 4.5;         % 'magThick [mm]'     
    varin(:,2) = x(:,2)*0.5 + 6.0;         % 'Tooth width [mm]'   
    varin(:,3) = x(:,3)*2 + 46.0;          % 'Magnet angle [deg]'
    varin(:,4) = x(:,4)*1 +146.0;         % 'Rotor outer diameter [mm]'
    varin(:,5) = x(:,5)*1 + 22.0;          % 'magLength[mm]'
    varin(:,6) = x(:,6)*2 + 31.0;          % 'Position of mag [mm]'
    varin(:,7) = x(:,7)*1 +18.0;           % 'CorebackWidth [mm]'
    varin(:,8) = 2.0e6;                       % 'current density [A/mm2]'
elseif isotest == 1  
    % used for debugging inputs for this script
    varin = [
    2.5e6  5.2  6.5
    2.0e6  6.0  5.5
    2.0e6  6.5  5.
...
%% C. image transfer
% python function to filter the obviously bad results
% Note in order to run conda environment dependent scripts, you need to start matlab from anaconda prompt 
% step1: generate images for NN model and record time consumed
ts=tic();
dlmwrite(inputfile, varfe(:,1:end-1));
Switch=1;       % 0 - run Optmodel, 1 - run image-transfer model
JMAGcontrol_v2(Switch, inputfile, datafile, root)   % remember to set file path in script
time = toc(ts);
% step2: image crop and image transfer. then check: if B<1.2T output 0, if B>=1.2T output 1. 
% remember to change image path in ImageCrop, ImageTransfer and
% ImageClassifer
system('python .\CommonCore\ImageTransfer\ImageCrop.py')
system('python .\CommonCore\ImageTransfer\ImageTransfer.py')
system('python .\CommonCore\ImageTransfer\ImageClassifier.py')
result=strcat(root,'checkpoint\ITClassified.csv');
out = csvread(result,1,0);
% save(strcat(num2str(gen),'.txt'),'out');
csvwrite(strcat(root,'\checkpoint\ITresult\gen',num2str(gen),'.csv'),out);
movefiles;      % move files into gen numbered folders 
% step3: pick the '1' case and generate FE input
[row, col]=size(out);
k=1;
out=sortrows(out,1);    % sort case No. in accend order
% give the input of '1' case to varfen
for n = 1:row
    if out(n,3)==1
        ind = out(n,1);
        varfen(k,:) = varfe(ind,:);
...
[caseNofen,varNo]=size(varfen);


%% D. put the filtered varfen into inputfile and run FEM
...
for n=1:caseNofen
    ...
    if n==1
        ...
        % input table for FEM
        stat_in = cat(2, repmat(varfen(n,1:end-1),length(Iw),1), Iu', Iv', Iw', ang');
    else
        update = cat(2, repmat(varfen(n,1:end-1),length(Iw),1), Iu', Iv', Iw', ang');
        stat_in = [stat_in; update];
    end
end
dlmwrite(inputfile, stat_in);
Switch=0;       % 0 - run Optmodel, 1 - run image-transfer model
JMAGcontrol_v2(Switch, inputfile, datafile, root)   % remember to set file path in script
...
len = length(ang);
Tm = zeros(caseNofen,1);
Tm_ang = zeros(caseNofen,1);
...
% expand the D matrix from varfen back to varfe size
m=1; k=1;
D=cat(2, zeros(caseNofe,1), varfe, zeros(caseNofe,1));
for n = 1:row
    if out(n,3)==1
       ind= out(n,1);
       D(ind,:)=cat(2, Tm(k), varfen(k,:), Tm_ang(k));
       k=k+1;
    end
end
